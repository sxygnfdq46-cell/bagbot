          ...
          <Card title="Multi-section browser" subtitle="Dial-in timeframe + asset contexts">
            <div className="flex flex-wrap items-center gap-3">
              <div className="stack-gap-xxs">
                <p className="metric-label">Timeframe</p>
            ...
            <div className="grid-premium mt-6 sm:grid-cols-2 lg:grid-cols-4">
              {overviewStats.map((stat) => (
                <div key={stat.label} className="info-tablet">
                  <p className="metric-label" style={{ color: stat.accent }}>
                    {stat.label}
                  </p>
              ...
              {miniCharts.map((chart) => (
                <div key={chart.symbol} className="dashboard-tile">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="metric-label text-[color:var(--accent-gold)]">{chart.symbol}</p>
            ...
            <div className="grid-premium lg:grid-cols-2">
              <div className="stack-gap-md">
                <div className="info-tablet">
                  <p className="metric-label text-[color:var(--accent-gold)]">Live graph</p>
                  <div className="mt-4 h-28">
                    {loading ? <Skeleton className="h-full w-full" /> : <Sparkline points={pulse} stroke="var(--accent-green)" height={90} />}
                  </div>
                </div>
                <div className="dashboard-tile">
                  <p className="metric-label">Payload cadence</p>
              ...
              <div className="surface-float grid gap-3">
                <p className="metric-label text-[color:var(--accent-gold)]">Feed log</p>
                <div className="no-scrollbar flex max-h-64 flex-col gap-3 overflow-y-auto">
                  {feed.map((event) => (
