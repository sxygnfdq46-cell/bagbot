...
const DEFAULT_TOOLS: ChartTool[] = ["off", "trendline", "horizontal"];

const LABELS: Record<ChartTool, string> = {
  off: "Off",
  trendline: "Trendline",
...
type ToolsSelectorProps = {
  tool: ChartTool;
  onSelect: (value: ChartTool) => void;
  options?: ChartTool[];
};

export default function ToolsSelector({ tool, onSelect, options }: ToolsSelectorProps) {
  const [open, setOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement | null>(null);
  const list = useMemo(() => (options && options.length ? options : DEFAULT_TOOLS), [options]);
  ...
  const handleSelect = (value: ChartTool) => {
    onSelect(value);
    setOpen(false);
  };
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        <Tag className="text-[11px] uppercase tracking-[0.22em]" variant="default">Tools</Tag>
        <span className="text-xs uppercase tracking-[0.08em] text-slate-200/90">{LABELS[tool]}</span>
      </button>
      ...
      {open ? (
        <div className="absolute left-0 top-full z-20 mt-2 min-w-[200px] overflow-hidden rounded-lg border border-white/10 bg-slate-900/90 p-2 shadow-xl backdrop-blur">
          <div className="mb-2 px-2">
            <MetricLabel className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Select tool</MetricLabel>
          </div>
          <div role="listbox" aria-label="Tools" className="space-y-1">
            {list.map((value) => {
